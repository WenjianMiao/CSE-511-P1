...
const char CHILD_ARRIVE[] = "ChildArrive", CHILD_LEAVE[] = "ChildLeave", 
	    ...
	    MODIFY_GIVER_NUM[] = "ModifyGiver", MODIFY_CHILD_NUM[] = "ModifyChild";

// TODO: Write the Path Expression solving the Child-care problem
//const char PATH_EXP[] = "path CaregiverArrive;(({(CaregiverArrive;CaregiverLeave) + (ChildArrive;ChildLeave)};CaregiverLeave) + CareLeave) end";
const char PATH_EXP[] = "path { (CaregiverArrive  ; CaregiverLeave ) + ( ChildArrive ; ChildLeave ) } end path {ModifyGiver} + {ModifyChild} end" ;

int k = 3; //This is the parameter that we need to solve the bonus. Specificlly, k = 3 here.
...
void caregiver_arrive(_thread_info_t *myinfo)
{
  ...
  // YOUR CODE HERE
  ENTER_OPERATION(MODIFY_GIVER_NUM);
  numgiver++;
  EXIT_OPERATION(MODIFY_GIVER_NUM);
...
{
  while(numchild+1 > k*numgiver){
	;
  }  
  ENTER_OPERATION(CHILD_ARRIVE);
  clock_gettime(CLOCK_MONOTONIC, &myinfo->cs_enter_time);

  ENTER_OPERATION(MODIFY_CHILD_NUM);
  
  numchild++;
  EXIT_OPERATION(MODIFY_CHILD_NUM);
  // YOUR CODE HERE
